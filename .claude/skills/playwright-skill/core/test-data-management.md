...
> **Flow-Guard generated specs:** Define the factory function inline at the top of the spec file — no separate import allowed. The file-based pattern further below is for human-authored test suites only.

**TypeScript — inline (for generated specs)**
```typescript
// Defined at the top of the spec file
function createTaskData(overrides: Partial<{ name: string; description: string }> = {}) {
  const id = Date.now();
  return {
    name: `Task-${id}`,
...
**TypeScript — file-based (for human-authored suites)**
```typescript
...
> **Flow-Guard generated specs:** Define the builder class inline at the top of the spec file — no separate import allowed.
...
**Strategy 0: Inline `test.afterEach` — preferred for Flow-Guard generated specs**

Use this when the spec is self-contained (no external fixture imports). Playwright's built-in `request` fixture inherits `storageState` from `playwright.config.ts`, so DELETE calls are already authenticated — no extra auth logic needed in the spec.

```typescript
test.describe('建立 Task 流程', () => {
  ...
  test('建立 Task 後應出現在列表中', async ({ page }) => {
    ...
    // Capture ID from URL or page attribute for cleanup
    await page.waitForURL(/\/tasks\/(\d+)/);
    createdTaskId = page.url().match(/\/tasks\/(\d+)/)![1];
...
Wrap the DELETE call in `.catch(() => {})` so a cleanup failure never masks the real test failure.

---

**Strategy 1: Fixture teardown (preferred for human-authored suites)**
...
## cases.md `cleanup:` Field (Flow-Guard)

When Phase A identifies that a test case creates persistent data, it adds a `cleanup:` field to that case in `cases.md`. Phase B reads this field and generates a `test.afterEach` block automatically.
...
test.describe('建立 Task 並確認出現在列表', () => {
  ...
  // ... test body captures createdTaskId during the flow
});
```
...
|---|---|---|
| `resource` | yes | Human-readable name of the resource being created |
| `id_from` | yes | Variable name that will hold the resource ID at runtime |
| `endpoint` | yes | DELETE path with `{id}` placeholder |
| `method` | no | HTTP method — defaults to `DELETE` |

See `CLAUDE.md § Test Data Cleanup` for the authoritative Phase A/B rules.
