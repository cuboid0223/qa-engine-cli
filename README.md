...
AI 驅動的 QA 測試系統，透過 Claude Code 與 Playwright MCP server，對 web 應用執行系統性流程驗證。
...
reports/
    ...
    baselines/baseline-<target>.json  ← /approve 產出的 regression baseline
...
AI 透過 MCP 瀏覽目標 app，結合 source code 和 PRD（若提供），產生 `cases.md`。
...
3. 用 MCP 點遍 role switcher 的每個選項（兩者都沒有時）
...
- MCP refs 轉換為穩定 selector（`getByRole` / `getByLabel` / `data-testid`）
...
```bash
npx playwright test <folder>/flow.spec.ts --reporter=junit,list
```
...
npx @playwright/mcp@latest --cdp-endpoint http://localhost:9222 --output-dir ./reports/screenshots
```
...
```json
{
  "mcpServers": {
    "playwright": {
      "command": "npx",
      "args": ["@playwright/mcp@latest", "--cdp-endpoint", "http://localhost:9222", "--output-dir", "./reports/screenshots"]
    }
  }
}
```
...
├── .mcp.json                     ← Playwright MCP 連接設定
├── playwright.config.ts          ← Playwright 執行設定
├── package.json
├── .env.example                  ← 憑證範本
│
├── .claude/
│   ├── settings.json             ← MCP 工具白名單
│   └── commands/
│       ├── plan.md               ← /plan
│       ├── generate.md           ← /generate
│       ├── test.md               ← /test
│       ├── run.md                ← /run
│       └── test-cases.md         ← cases.md 格式規範（Phase A 讀取）
│
├── tests/
